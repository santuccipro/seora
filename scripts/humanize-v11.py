#!/usr/bin/env python3
"""
humanize-v11.py — Humanisation DIRECTEMENT dans le PDF via PyMuPDF.
- Pas de DOCX, pas d'HTML, pas de Chrome.
- Mise en page préservée à 100% : seul le texte IA change.
- Redact + insert_textbox sur les blocs ciblés.

Usage:
  python3 humanize-v11.py input.pdf [--output out.pdf] [--threshold 25] [--lang fr]
"""

import sys, os, json, re, time, argparse, subprocess, tempfile, difflib
...
def extract_text_blocks(pdf_path: str) -> list:
    """Extrait les blocs texte avec position, taille de police et contenu."""
    import fitz
...
def find_ai_blocks(blocks: list, chunk_results: list, threshold: float) -> set:
    """Mappe les résultats de détection aux indices de blocs."""
    # Construire la liste de textes et leurs offsets dans le texte global
...
def humanize_blocks_targeted(blocks: list, target_idx: list, signals_prompt: str, lang: str) -> tuple:
    """Humanise les blocs ciblés. Retourne (blocks_mis_a_jour, diff_log)."""
    if not target_idx:
        return blocks, []

    texts = [blocks[i]["text"] for i in target_idx]
    ...
    for j, i in enumerate(target_idx):
        ...
        if new_t and new_t != orig_t and len(new_t) > 5:
            ratio = len(new_t) / max(len(orig_t), 1)
            if 0.5 <= ratio <= 1.8:
                result[i]["new_text"] = new_t
                diff_log.append({"page": blocks[i]["page"], "before": orig_t, "after": new_t})
            else:
                print(f"  Bloc {i} : longueur trop différente ({ratio:.1f}x), on garde l'original")
                result[i]["new_text"] = orig_t
        else:
            result[i]["new_text"] = orig_t
...
def get_dominant_font(block: dict) -> str:
    """Récupère le nom de la police dominante dans un bloc."""
    fonts = {}
    ...
    # Normaliser vers une police embarquée PyMuPDF (noms corrects)
    nl = name.lower()
...
def get_text_alignment(block: dict) -> int:
    """Détecte l'alignement (0=left, 1=center, 2=right)."""
    # Heuristique : si le bloc est centré sur la page (x0 > 100mm et x1 < page_width-100mm)
    x0, y0, x1, y1 = block["bbox"]
    width = x1 - x0
    # Si très court et au milieu de la page, probable centré
    if width < 200 and x0 > 100:
        return 1  # centré
    return 0  # gauche par défaut

def apply_replacements_to_pdf(pdf_path: str, blocks: list, output_path: str):
    """
    Remplace le texte in-place dans le PDF :
    1. Redact étendu (efface les blocs modifiés avec marge)
    2. Re-insère le nouveau texte avec la bonne police + clean_contents
    """
    import fitz
    ...
    for b in blocks:
        ...
        if new_t != b["text"]:  # seulement si modifié
            p = b["page"]
            if p not in by_page:
                by_page[p] = []
            by_page[p].append(b)
    ...
    for page_num, page_blocks in by_page.items():
        ...
        # Passe 1 : redact avec marge de 1pt pour éviter les artefacts de bord
        for b in page_blocks:
            rect = fitz.Rect(b["bbox"])
            # Expand légèrement pour capturer les glyphes aux bords
            rect_exp = rect + (-1, -1, 1, 1)
            page.add_redact_annot(rect_exp, fill=(1, 1, 1), text="")

        # Appliquer les redactions (supprime texte + dessins dans la zone)
        page.apply_redactions(images=fitz.PDF_REDACT_IMAGE_NONE)

        # Passe 2 : réinsérer le texte humanisé
        for b in page_blocks:
            rect = fitz.Rect(b["bbox"])
            new_t = b.get("new_text", b["text"])
            # Nettoyer les caractères invisibles dans le nouveau texte
            new_t = re.sub(r"[​‌‍­﻿⁠\x00-\x08\x0b\x0c\x0e-\x1f]", "", new_t)
            ...
            n_replaced += 1
            if rc < 0:
                print(f"  ⚠️  Bloc p{page_num} : texte tronqué (rc={rc}), font_size réduite")
                # Retry avec font_size réduite de 0.5pt
                page.insert_textbox(
                    rect, new_t,
        ...
        # Nettoyer le content stream de la page pour éliminer les résidus
        try:
            page.clean_contents()
        except Exception:
            pass

    print(f"  {n_replaced} blocs remplacés in-place")
...
def word_diff_html(before: str, after: str) -> tuple:
    """Returns (before_html, after_html) with changed words highlighted."""
    bw = before.split()
    aw = after.split()
...
def generate_diff_report(diff_log: list, out_path: str, score_before: float, score_after: float) -> str:
    """Generate HTML avant/après report. Returns path to the HTML file."""
...
def main():
    parser = argparse.ArgumentParser(description="humanize-v11 — remplacement PDF in-place")
    ...
    if not args.input.lower().endswith(".pdf"):
        print("ERREUR : humanize-v11 accepte uniquement des fichiers PDF")
        sys.exit(1)
    ...
    out_path = args.output or f"{base}_humanise_v11.pdf"

    print(f"\n{'='*60}")
    print(f"  humanize-v11 | {os.path.basename(args.input)} → {os.path.basename(out_path)}")
    print(f"  Seuil: {args.threshold}% | Langue: {args.lang}")
    print(f"  Mode: PDF in-place (PyMuPDF redact + insert)")
    print(f"{'='*60}")

    # 1. Extraction blocs
    print(f"\n[1/6] Extraction blocs PDF...")
    blocks = extract_text_blocks(args.input)
    # Filtrer les blocs à ne pas toucher
    eligible = [b for b in blocks if not should_skip(b["text"])]
    ...
    # Texte complet (après fixes directs)
    def block_text(b):
    ...
    if ai_idx:
        ...
        print(f"\n[4/6] Humanisation : rien à faire (LLM)")

    # 5. Remplacement in-place dans le PDF
    print(f"\n[5/6] Remplacement in-place dans le PDF...")
    # Reconstruire la liste complète avec les modifications
    # (les blocs non-éligibles restent intouchés dans le PDF)
    all_text_after = "\n".join(block_text(b) for b in eligible)
    apply_replacements_to_pdf(args.input, eligible, out_path)
    ...
    if final_score > TARGET_SCORE:
        ...
        if from_slow:
            ...
            apply_replacements_to_pdf(out_path, eligible, tmp_p2)
    ...
    # Génération rapport avant/après
    report_path = generate_diff_report(diff_log, out_path, init_score, final_score)
    # Note: final_score est en fast_mode — pour score slow_mode précis, lancer detect() manuellement
