#!/usr/bin/env python3
"""
humanize-v12.py — Humanisation DIRECTEMENT dans le PDF via PyMuPDF.
...
- v12 : JSON parsing robuste, prompt LLM agressif, rapport pré-analyse IA, logging amélioré.

Usage:
  python3 humanize-v12.py input.pdf [--output out.pdf] [--threshold 25] [--lang fr]
"""
...
# ─── LLM rewriter (v12 : JSON parsing robuste + prompt agressif) ─────────────

def _parse_llm_json(raw: str) -> list:
    """Tente de parser le JSON LLM avec 3 stratégies de fallback."""
    ...
    # Stratégie 2 : extraire le tableau JSON (greedy, multiline)
    ...
    if m:
        try:
            ...
            pass
        # Stratégie 3 : réparer le JSON tronqué (ajouter `]` si manquant)
        try:
            items = json.loads(m.group(0) + ']')
            if isinstance(items, list):
                return items
...
def humanize_blocks_targeted(blocks: list, target_idx: list, signals_prompt: str, lang: str) -> tuple:
    ...
    # Fix v12 : utilise new_text (direct-fixed) si disponible, pas le texte original brut
    ...
    for j, i in enumerate(target_idx):
        ...
        src_t = blocks[i].get("new_text", orig_t)  # texte envoyé au LLM
...
# ─── Rapport pré-analyse IA (v12) ────────────────────────────────────────────

def generate_analysis_report(blocks: list, detect_result: dict, ai_idx: set, out_path: str) -> str:
    """Génère un rapport HTML montrant quels blocs sont détectés comme IA."""
...
def main():
    parser = argparse.ArgumentParser(description="humanize-v12 — remplacement PDF in-place")
    ...
    if not args.input.lower().endswith(".pdf"):
        print("ERREUR : humanize-v12 accepte uniquement des fichiers PDF")
        sys.exit(1)
    ...
    out_path = args.output or f"{base}_humanise_v12.pdf"

    print(f"\n{'='*60}")
    print(f"  humanize-v12 | {os.path.basename(args.input)} → {os.path.basename(out_path)}")
