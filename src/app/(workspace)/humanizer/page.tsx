...
export default function HumanizerPage() {
  ...
  const [docxRewordifyLoading, setDocxRewordifyLoading] = useState(false);
  ...
  const startDocxRewordify = useCallback(async () => {
    if (!uploaded) return;
    setDocxRewordifyLoading(true);
    try {
      const fd = new FormData();
      fd.append("file", uploaded.file);
      fd.append("language", language);
      ...
      if (scores && scores.length > 0) {
        fd.append("paragraphScores", JSON.stringify(scores));
      }

      const res = await fetch("/api/humanize/docx-rewordify", {
        method: "POST",
        body: fd,
      });
      if (!res.ok) {
      ...
      const rewrittenCount = res.headers.get("X-Rewritten-Count") ?? "?";
      const blob = await res.blob();
      ...
      toast.success(`DOCX téléchargé · ${rewrittenCount} zones réécrites (Rewordify − 3 tokens)`);
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Erreur téléchargement DOCX");
    } finally {
      setDocxRewordifyLoading(false);
    }
  }, [uploaded, language, analyzeOnlyResultV2, analyzeOnlyResult]);
